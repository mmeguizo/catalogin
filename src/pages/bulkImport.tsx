...
export default function BulkImportsPage() {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (jsonData.length === 0) {
          setMessage({
            type: "info",
            text: "The Excel file is empty or has no data rows after the header.",
          });
          setIsLoading(false);
          return;
        }
        ...
        const booksToProcess: Partial<Book>[] = jsonData.map(
          (row: any, rowIndex: number) => {
            ...
            for (const field of booksDataSource.fields) {
              ...
              const foundHeaderKey = rowKeys.find(
                (key) =>
                  key.toLowerCase() === String(headerNameFromDef).toLowerCase()
              );
              if (foundHeaderKey) {
              ...
              if (
                ...
                String(fieldName).toLowerCase() !==
                  String(headerNameFromDef).toLowerCase()
              ) {
                const foundFieldKey = rowKeys.find(
              ...
              if (
                ...
                switch (
                  field.type // field.type is a UI hint, but we use it for basic conversion
                ) {
                  case "number":
                    ...
                    if (parsedValue === undefined) {
                      console.warn(
                        `Row ${rowIndex + 2}, Field "${String(fieldName).toLowerCase()}": Could not parse "${String(excelValue)}" as number.`
                      );
                    }
                    break;
                  case "date": // Assuming Zod expects a string like 'YYYY-MM-DD' for date fields like 'date_added'
                    ...
                    if (
                      ...
                      // Common range for Excel dates
                      tempDate = new Date(
                        Math.round((excelValue - 25569) * 86400 * 1000)
                      );
                    } else {
                      // Handle string dates
                    ...
                    if (tempDate && !isNaN(tempDate.getTime())) {
                      ...
                      console.warn(
                        `Row ${rowIndex + 2}, Field "${String(fieldName).toLowerCase()}": Could not parse "${String(excelValue)}" as date string.`
                      );
                    }
                    break;
                  case "boolean":
                    parsedValue = ["true", "1", "yes", "on"].includes(
        ...
        if (booksToProcess.length === 0) {
          setMessage({
            type: "info",
            text: "No data rows found in the Excel file to import.",
          });
          setIsLoading(false);
          return;
        }
        ...
        try {
          ...
          if (Array.isArray(allBooks)) {
            ...
            console.log(
              `[DEBUG] Fetched ${existingAccessionNumbers.length} existing accession numbers for duplicate checking`
            );
          } else {
            console.error(
              "[DEBUG] Expected allBooks to be an array but got:",
              typeof allBooks
            );
          ...
          console.error(
            "[DEBUG] Error fetching existing accession numbers:",
            error
          );
          ...
          setMessage({
            type: "error",
            text:
              "Failed to check for duplicates. This could be due to a database connection issue or server error. Please try again later or contact your system administrator if the problem persists. Error details: " +
        ...
        for (let i = 0; i < booksToProcess.length; i++) {
          ...
          if (
            !bookInput.Accession_Number ||
            String(bookInput.Accession_Number).trim() === ""
          ) {
            errors.push(
              `Row ${excelRowNumber} (Title: ${bookInput.Title || "N/A"}): Missing Accession Number.`
            );
          ...
          if (existingAccessionNumbers.includes(accessionNumber)) {
            errors.push(
              `Row ${excelRowNumber} (Title: ${bookInput.Title || "N/A"}): Duplicate Accession Number "${accessionNumber}" already exists in the database.`
            );
            duplicateCount++;
            validationErrorCount++;
            continue;
          ...
          console.log(
            `[DEBUG] Raw input for Excel row ${excelRowNumber} (Title from input: ${bookInput.Title || "N/A"}):`,
            JSON.parse(JSON.stringify(bookInput))
          );
          console.log(
            `[DEBUG] Excel row ${excelRowNumber}: Accession_Number from input: "${bookInput.Accession_Number}" (Type: ${typeof bookInput.Accession_Number})`
          );
          ...
          if (booksDataSource.validate) {
            try {
              ...
              const validatedDataFromZod = await booksDataSource.validate(
                bookInput as Partial<Omit<Book, "id">>
              );
              // If it reaches here, validation was successful.
              // Toolpad's ['~standard'].validate might wrap data in a 'value' property.
              validatedBookData =
                (validatedDataFromZod as any)?.value || validatedDataFromZod;
              console.log(
                `[DEBUG] Excel row ${excelRowNumber}: ZodValidation PASSED. Validated data (potentially unwrapped):`,
                JSON.parse(JSON.stringify(validatedBookData))
              );
            } catch (error: any) {
              ...
              validationErrorCount++;
              console.log(
                `[DEBUG] Excel row ${excelRowNumber}: Validation FAILED. Caught error:`,
                error
              );
              ...
              if (error.name === "ZodError" && error.issues) {
                console.log(
                  `[DEBUG] ZodValidation FAILED for Excel row ${excelRowNumber}. Error object has 'issues' array:`,
                  JSON.stringify(error.issues, null, 2)
                );
                validationIssues = error.issues;
              } else if (typeof error.message === "string") {
                console.log(
                  `[DEBUG] ZodValidation FAILED for Excel row ${excelRowNumber}. Error object has 'message':`,
                  error.message
                );
                ...
                console.log(
                  `[DEBUG] ZodValidation FAILED for Excel row ${excelRowNumber}. Error object is not standard ZodError:`,
                  error
                );
                const errorMessage =
                  typeof error?.message === "string"
                    ? error.message
                    : "Unknown validation error structure caught.";
                validationIssues = [
                  { path: ["unknown"], message: errorMessage },
                ];
              }
            }
          } else {
            ...
            console.warn(
              `[DEBUG] No validation method defined in booksDataSource. Proceeding with raw input for row ${excelRowNumber}.`
            );
                ...
                .map((issue: any) => {
                  const fieldKey =
                    ...
                    issue.path.length > 0
                      ? issue.path[0]
                      : "unknown_field";
                  const fieldDefinition = booksDataSource.fields.find(
                    (f) => f.field === fieldKey
                  );
                  // Use headerName for user-friendly display, fallback to fieldKey with spaces
                  const displayName =
                    fieldDefinition?.headerName || fieldKey.replace(/_/g, " ");
                  return `Column '${displayName}': ${issue.message}`;
                })
                .join("; ") || "Unknown validation issue(s). Check console.";
            console.log(
              `[DEBUG] Excel row ${excelRowNumber}: Formatted field errors for UI: "${fieldErrors}"`
            );
            errors.push(
              `Row ${excelRowNumber} (Title: ${bookInput.Title || "N/A"}): Validation failed - ${fieldErrors}`
            );
          ...
          if (validatedBookData) {
            if (!booksDataSource.createOne) {
              console.error(
                "[DEBUG] booksDataSource.createOne method is not defined."
              );
              throw new Error(
                "booksDataSource.createOne method is not defined. Cannot create book."
              );
            }
            try {
              // Log the exact data payload being sent to createOne
              console.log(
                `[DEBUG] Attempting to create book for Excel row ${excelRowNumber} with data payload:`,
                validatedBookData
              );
              await booksDataSource.createOne(
                validatedBookData as Omit<Book, "id">
              );
              ...
              successCount++;
              console.log(
                `[DEBUG] Excel row ${excelRowNumber}: Successfully created.`
              );
            } catch (creationError: any) {
              creationErrorCount++;
              // Log errors that occur during the createOne database operation
              errors.push(
                `Row ${excelRowNumber} (Title: ${bookInput.Title || "N/A"}): Creation failed - ${creationError.message || "Unknown error"}`
              );
              console.error(
                `[DEBUG] Error during createOne for Excel row ${excelRowNumber} (Title: ${bookInput.Title || "N/A"}):`,
                creationError,
                "Data sent:",
                validatedBookData
              );
            ...
            // Should only be reached if booksDataSource.validate was undefined and we chose to not fail
            validationFailed = true;
            validationErrorCount++;
            errors.push(
              `Row ${excelRowNumber} (Title: ${bookInput.Title || "N/A"}): Skipped due to missing validation outcome or data.`
            );
            continue; // Skip this book due to validation errors
          }
        }
        ...
        if (duplicateCount > 0) {
          const duplicateErrors = errors.filter((err) =>
            err.includes("Duplicate Accession Number")
          );
          ...
          if (errors.length > 0) {
            ...
            setMessage({
              type: "info",
              text: `No books were imported. Issues found:\n- ${errorDetails}${errors.length > 15 ? "\n... (more errors in console)" : ""}`,
            });
          } else {
            setMessage({
              type: "info",
              text: "No valid book data found to import.",
            });
          }
        } else {
        ...
        // Errors from XLSX.read or initial file processing
        console.error("Error processing Excel file:", parseError);
        setMessage({
          type: "error",
          text: `Error processing Excel file: ${parseError.message}`,
        });
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <PageContainer title="Bulk Import Books" breadcrumbs={[]}>
      <Paper sx={{ p: 3, maxWidth: "lg", margin: "auto" }}>
        {" "}
        {/* Increased maxWidth for better error display */}
        <Typography variant="h5" component="h1" gutterBottom>
          Import Books from Excel
        </Typography>
        <Typography variant="body2" gutterBottom sx={{ mb: 2 }}>
          Ensure Excel column headers match the field names or headers defined
          in the system (e.g., "Accession Number", "Title", "ISBN"). The import
          will attempt case-insensitive matching.
        </Typography>
        <Stack
          spacing={3}
          component="form"
        ...
        >
          ...
          {message && (
            ...
            >
              {" "}
              {/* Allow scrolling for long messages */}
              {message.text}
            </Alert>
          )}
