...
export default function DashboardPage() {
      ...
      try {
        if (booksDataSource && booksDataSource.getMany) {
          ...
          if (result && Array.isArray(result.items)) {
            ...
            setBooks([]);
            console.warn(
              "Dashboard: booksDataSource.getMany did not return expected array of items."
            );
          }
        } else {
          throw new Error(
            "Books data source or its getMany method is not available."
          );
        }
      } catch (err: any) {
    ...
    books.forEach((book) => {
      if (book.copyright_year !== undefined && book.copyright_year !== null) {
        ...
        if (!isNaN(year)) {
          ...
          console.warn(
            "Dashboard: Invalid copyright_year found:",
            book.copyright_year,
            "for book ID:",
            book.id
          );
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {books.length === 0 ? (
        <Alert severity="info">
          No book data available to display charts. Add some books!
        </Alert>
      ) : (
        <React.Fragment>
          ...
          <Paper elevation={3} sx={{ p: 2, mb: 4 }}>
            ...
            {years.length > 0 && booksByYear[0].data.length > 0 ? (
              <BarChart
                xAxis={[
                  { scaleType: "band", data: years, label: "Copyright Year" },
                ]}
              ...
              <Typography variant="body2" sx={{ textAlign: "center", p: 2 }}>
                No copyright year data to display. Ensure books have valid
                'Copyright Year'.
              </Typography>
            )}
          </Paper>
          ...
          <Paper elevation={3} sx={{ p: 2 }}>
            ...
            {booksByDDC.length > 0 ? (
              ...
              <Typography variant="body2" sx={{ textAlign: "center", p: 2 }}>
                No DDC classification data to display. Ensure books have valid
                'DDC'.
              </Typography>
            )}
          </Paper>
